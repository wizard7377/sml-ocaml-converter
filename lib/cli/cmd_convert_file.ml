...
let run_cmd : int Cmd.t =
  ...
  @@ let+ output = output
     ...
     Toplevel.convert_file ~options:common_options ?output_file:(Option.map Toplevel.string_to_path output) ~input_files:(List.map Toplevel.string_to_path input)
...
let output_dir : string Term.t =
  ...
  |} in
  Arg.(required & opt (some string) None & info [ "output" ] ~doc ~docv:"OUTPUT_DIR")
let input_dir : string Term.t =
  ...
  |} in
  Arg.(required & opt (some string) None & info [ "input" ] ~doc ~docv:"INPUT_DIR")
let run_cmd : int Cmd.t =
  let doc = {|
  ...
  @@ let+ output_dir = output_dir
     ...
     and+ common_options = common_options in
     Toplevel.convert_group ~options:common_options ~output_dir:(Toplevel.string_to_path output_dir) ~input_dir:(Toplevel.string_to_path input_dir)
end
