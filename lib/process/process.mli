(** Conversion pipeline orchestration.
    ...
    conversion process. It coordinates the lexer, parser, backend transformation,
...
(** Main conversion processor class.
    ...
    @param store Optional initial name store. If not provided, a fresh store
                 is created. Useful for pre-populating with standard library
...
class process : ?store:Names.Store.t -> config -> object
