...
let display_error_context ?(context=4) (filename:string) (p0:Lexing.position) (p1:Lexing.position) = 
  let path' = Filename.current_dir_name ^ "/" ^ filename in
  let file_content =
    try
      Bos.OS.File.read_lines (Fpath.v path') |> Result.get_ok
    with _ -> []
  in 
  ...
  let contents = List.filteri (fun i _ -> (i >= start_line && i <= end_line)) file_content in
  let contents0 = List.mapi (fun i line -> 
    let line_no = i + start_line + 1 in
    if line_no = p0.pos_lnum then
      Printf.sprintf ">> %4d | %s" line_no line
    else
      Printf.sprintf "   %4d | %s" line_no line
  ) contents in
...
let check_output ~(config : Common.options) ?(output_file= get_output_file config)  (input : string) : check_result =
  ...
  with 
    ...
    let file_content = display_error_context loc.loc_start.pos_fname loc.loc_start loc.loc_end in
