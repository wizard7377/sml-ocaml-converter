(** Common utilities for output validation and error reporting.
    ...
    This module provides functionality for validating generated OCaml code
    and formatting error messages with source context. *)
...
(** [check_output ~config ?output_file ocaml_code] validates OCaml syntax.
    ...
    Parses the generated OCaml code using the OCaml compiler's parser to
    detect syntax errors. If errors are found, provides detailed context
    showing the problematic lines.

    @param config Conversion configuration (for input file names in error messages)
    ...
    @return Validation result: Good, Bad with error details, or Err for other failures *)
