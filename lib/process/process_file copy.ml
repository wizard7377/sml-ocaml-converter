...
class process_file ?(storecfg_init = Names.Store.create []) cfg_init =
  object (self)
    val mutable cfg = cfg_init
    val mutable store : Names.Store.t = storecfg_init
    method get_store () : Names.Store.t = store
    method set_store (s : Names.Store.t) = store <- s
    ...
    method parse_sml (s : string) : sml_code = Frontend.parse s

    method convert_to_ocaml (sml : sml_code) : ocaml_code =
      let module Backend = Backend.Make (struct
...
let run (self : process) : int =
  try
    let module Backend_S = struct
      let config = self#get_config
    end in
    let module Backend = Backend.Make (Backend_S) in
    ...
    let () =
      List.iter (Pprintast.top_phrase (get_fmt self#get_config)) ocaml_code
    in
    0
  with e ->
