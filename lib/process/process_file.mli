(** Single-file conversion processing.
    ...
    This module provides the core pipeline for converting individual SML source
    files to OCaml. It handles the complete transformation from raw SML text to
    formatted OCaml output.
    ...
    Each phase is exposed as a separate method, allowing fine-grained control
    over the conversion process.

    {1 Architecture}

    The {!process_file} class extends the general processing framework with
    file-specific operations. It maintains:
    - Configuration state
...
(** Internal representation of SML source code.
    ...
    This abstract type holds the parsed SML AST along with any metadata needed
    for conversion. *)
...
(** Internal representation of OCaml source code.
    ...
    This abstract type holds the OCaml Parsetree representation ready for
    pretty-printing. *)

(** File conversion processor class.

    This class provides a stateful pipeline for converting individual SML files
    to OCaml, with separate methods for each conversion phase.

    {3 Constructor Parameters}

    @param store
      Optional initial name store for pre-populating with known identifiers
      (e.g., standard library names)
    @param config
...
class process_file : ?store:Context.t -> Common.t -> object
  ...
  (** [get_store ()] retrieves the accumulated name store.
      ...
      The store contains all identifiers discovered during parsing and their
      syntactic contexts. Useful for:
  ...
  (** [convert_to_ocaml sml] transforms SML AST to OCaml Parsetree.
      ...
      The conversion respects the configuration flags to enable/disable specific
      transformations.
