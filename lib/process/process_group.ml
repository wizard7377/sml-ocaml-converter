...
exports = if Sys.file_exists (group_file ^ ".sml") then Some (group_file ^ ".sml") else None ;
    ...
    method parse_members (members : t) : string = 
      try 
        let p0, p1, p2 = new Process_file.process_file cfg_init, new Process_file.process_file cfg_init, new Process_file.process_file cfg_init in
        let interface_output = match members.interface with
        | Some path -> p0#process_file (Result.get_ok (Bos.OS.File.read @@ Fpath.v path))
        | None -> "" in
        let implementation_output = match members.implementation with
        | Some path -> p1#process_file (Result.get_ok (Bos.OS.File.read @@ Fpath.v path))
        | None -> "" in
        let exports_output = match members.exports with
        | Some path -> p2#process_file (Result.get_ok (Bos.OS.File.read @@ Fpath.v path))
        | None -> "" in
        interface_output ^ "\n" ^ implementation_output ^ "\n" ^ exports_output
      with 
        e -> 
          Format.eprintf "Error: %s in processing group members.@." (Printexc.to_string e);
          raise e
        end 
