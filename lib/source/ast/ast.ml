(** {1 SML Abstract Syntax Tree}
    ...
    This is the main entry point for the AST library. It re-exports all types
    from sub-modules for backward compatibility with existing code.
    ...
    All types and functions from the original monolithic [Ast] module are
    re-exported here. Existing code using [Ast.type_name] or [open Ast] will
