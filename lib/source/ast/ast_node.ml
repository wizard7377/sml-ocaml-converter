(** {1 AST Node Wrapper}
    ...
    Each AST node can preserve its original source position for precise error
    reporting. When the parser creates nodes, it attaches the location from the
    lexer. When constructing AST programmatically (e.g., in tests or code
    generation), nodes have no position ([None]).
    ...
    {3 Uniform Traversal}
    Generic tree-walking functions can operate on any ['a node] without knowing
    the specific AST type, enabling reusable traversal libraries. *)

type 'a node = {
  value : 'a;
      (** The wrapped AST payload.

          This is the actual AST constructor value. Pattern match on this field
          to examine the AST structure. *)
  pos : (Lexing.position * Lexing.position) option; [@opaque]
      (** Optional source location (start position, end position).

          Set by the parser to track where this AST node originated in the
          source file. Used for error reporting with precise line and column
          information.
          ...
          {[
            ...
            | None -> Printf.printf "Programmatically created node\n"
          ]}
...
(** Create a node wrapper with no source position.
    ...
    Nodes created this way have [pos = None], indicating they don't correspond
    ...
    {[
      ...
      let nil_pattern =
        box_node
          (PatIdx (box_node (WithoutOp (box_node (IdxIdx (box_node "nil"))))))
    ]}
...
(** Extract the value from a node wrapper, discarding metadata.
    ...
    Use when pattern matching on AST contents. This is the primary way to
    destructure AST nodes and access their constructors.
    ...
    The typical pattern when processing AST nodes: 1. Use [unbox_node] to
    extract the constructor 2. Pattern match on the constructor 3. Recursively
    process child nodes (which are also wrapped) 4. Build new nodes using
    [box_node] if transforming

    {3 Note on Metadata Loss}

    This function discards source position information. If you need to preserve
    or examine positions, access [n.pos] directly instead of using this
    function.
