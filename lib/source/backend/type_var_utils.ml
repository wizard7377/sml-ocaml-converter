...
(** Process a type variable name and return an OCaml type variable.
    ...
    Converts SML type variables ('a, ''a) to OCaml Parsetree type variables.
    The leading quotes are stripped since OCaml's ptyp_var expects just
    the variable name without the quote. *)
...
(** Convert a list of SML type variable nodes to OCaml type parameters.
    ...
    Returns pairs of (core_type, variance/injectivity) suitable for
