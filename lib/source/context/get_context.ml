...
let rec process_con_specification ~(path : string list) ~(type_name : string)
  ...
  | ConDesc (name_node, typ_opt, rest_opt) ->
      ...
      let arity =
        match typ_opt with
        | Some ty -> (
            match unbox_node ty with
            | TypTuple tys -> Some (List.length tys)
            | _ -> Some 1)
        | Some _ -> Some 1
        | None -> Some 0
      in
      let info : Info.name_info = Info.ConstructorInfo { arity } in
      let rest =
        match rest_opt with
...
let rec process_dat_specification ~(path : string list)
    (dat_spec : dat_specification node) : Info.t =
...
and process_str_specification ~(path : string list)
    (str_spec : str_specification node) : Info.t =
...
let rec process_constructor_binding ~(path : string list) ~(type_name : string)
    (con_bind : constructor_binding node) : Info.t =
  match unbox_node con_bind with
  | ConBind (name_node, typ_opt, rest_opt) ->
      let name = idx_to_string (unbox_node name_node) in
      let arity = match typ_opt with Some _ -> Some 1 | None -> Some 0 in
      let info : Info.name_info = Info.ConstructorInfo { arity } in
      let full_name : Info.name = { path; root = name } in
...
let rec process_data_binding ~(path : string list)
    (dat_bind : data_binding node) : Info.t =
...
let rec process_structure ~(path : string list) (str : structure node) : Info.t
  ...
  | LocalDec (dec, str_inner) ->
      Info.merge
        (process_declaration ~path dec)
        (process_structure ~path str_inner)

(** Process a structure_binding node *)
and process_structure_binding ~(path : string list)
    (str_bind : structure_binding node) : Info.t =
...
let rec process_functor_binding ~(path : string list)
    (fct_bind : functor_binding node) : Info.t =
  match fct_bind.value with
...
let rec process_signature_binding ~(path : string list)
    (sig_bind : signature_binding node) : Info.t =
