...
*)
let rec process_con_specification ~(path : string list) ~(type_name : string) (con_spec : con_specification node) : Info.t =
...
let rec process_dat_specification ~(path : string list) (dat_spec : dat_specification node) : Info.t =
...
let rec process_specification ~(path : string list) (spec : specification node) : Info.t =
  ...
  | SpecSeq (s1, s2) ->
    Info.merge (process_specification ~path s1) (process_specification ~path s2)
...
and process_str_specification ~(path : string list) (str_spec : str_specification node) : Info.t =
...
and process_signature ~(path : string list) (sig_node : signature node) : Info.t =
  ...
  | SignSig specs ->
    specs |> List.fold_left (fun acc spec -> Info.merge acc (process_specification ~path spec)) (Info.create [])
  | SignWhere (sig_inner, _) -> process_signature ~path sig_inner

(** Process a constructor_binding node (for datatypes in declarations) *)
let rec process_constructor_binding ~(path : string list) ~(type_name : string) (con_bind : constructor_binding node) : Info.t =
  ...
  | ConBind (name_node, typ_opt, rest_opt) ->
    ...
    let info = Info.create [full_name, info] in
    let rest = match rest_opt with
...
let rec process_data_binding ~(path : string list) (dat_bind : data_binding node) : Info.t =
...
and process_declaration ~(path : string list) (dec : declaration node) : Info.t =
  ...
  | AbstractDec (dat_bind, _, decs) ->
    Info.merge (process_data_binding ~path dat_bind) (decs |> List.fold_left (fun acc dec -> Info.merge acc (process_declaration ~path dec)) (Info.create []))
  | ExnDec _ -> Info.create []
  | StrDec str_bind -> process_structure_binding ~path str_bind
  | SeqDec decs -> decs |> List.fold_left (fun acc dec -> Info.merge acc (process_declaration ~path dec)) (Info.create [])
  | LocalDec (dec1, dec2) ->
...
let rec process_functor_binding ~(path : string list) (fct_bind : functor_binding node) : Info.t =
...
let rec process_signature_binding ~(path : string list) (sig_bind : signature_binding node) : Info.t =
...
and process_prog ~(path : string list) (prog : Ast.prog) : Info.t =
  match prog with
