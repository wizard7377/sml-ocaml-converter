...
module Make_TestBackend (Ctx : sig val lexbuf : string end) = Backend.Make(struct let lexbuf = Ctx.lexbuf ;; let context = Context.create [] end)(TestConfig)



let process (before:string) : string =
  let middle = Re.replace ~all:true (Re.compile (Re.rep1 Re.space)) ~f:(fun _ -> " ") before in
  let after = Re.replace ~all:true (Re.compile (Re.str ";;")) ~f:(fun _ -> "") middle in
  String.trim after
...
module Make (Files : TEST_FILES) : TEST_CASE = struct
  ...
  let run_test : unit test_case = test_case test_name `Quick (fun () ->
    ...
    let module TestBackend = Make_TestBackend(struct let lexbuf = input end) in
    ...
    let test_name' = Re.replace ~all:true (Re.compile (Re.str " ")) ~f:(fun _ -> "_") test_name in
    Format.fprintf Format.err_formatter "@,@,@[Running test: @[%s@]@]@,@," test_name' ;
    Format.fprintf Format.err_formatter "@,@,@[Processed: %s@]@,@," processed_actual ;
