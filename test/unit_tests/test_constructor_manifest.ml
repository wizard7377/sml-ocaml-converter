...
let test_roundtrip () =
  ...
  let json = Context.Constructor_manifest.to_json constructors in
  let parsed = Context.Constructor_manifest.from_json json in

  check int "same count" 2 (List.length parsed);
  check string "first name" "SOME" (List.nth parsed 0).Context.Constructor_registry.name;
  check string "second ocaml_name" "Cons_" (List.nth parsed 1).Context.Constructor_registry.ocaml_name
...
let test_write_and_read (switch, temp_dir) =
  ...
  check string "loaded name" "Foo" (List.nth loaded 0).Context.Constructor_registry.name
